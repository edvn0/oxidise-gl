...
fn main() {
    ...
    for round in 0..ROUNDS {
        ...
        assert_eq!(
            world.len() as usize,
            ENTITIES,
            "round {round}: wrong entity count after spawn"
        );
        ...
        assert_eq!(
            post_tick as usize, ENTITIES,
            "round {round}: entity count changed during ticks ({post_tick} != {ENTITIES})"
        );
        eprintln!(
            "[round {round}] {TICKS} ticks × {ENTITIES} entities  —  {tick_ms} ms  ({:.1} ms/tick)",
        ...
        assert_eq!(
            world.len() as usize,
            ENTITIES / 2,
            "round {round}: entity leaked after partial despawn"
        );
        ...
        assert_eq!(
            world.len(),
            0,
            "round {round}: world not empty after full despawn"
        );
        eprintln!("[round {round}] despawn: all {ENTITIES} removed, world empty ✓");
    }

    eprintln!(
        "\nAll {ROUNDS} rounds passed in {:.2?}",
        total_start.elapsed()
    );
}
