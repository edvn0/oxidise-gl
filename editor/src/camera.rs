...
enum CaptureMode { None, Fly, Orbit, Pan }
...
impl EditorCamera {
    ...
    pub fn is_capturing(&self) -> bool { self.capture != CaptureMode::None }
    ...
    pub fn on_scroll(&mut self, delta: &MouseScrollDelta, viewport_hovered: bool) {
        if !viewport_hovered && !self.is_capturing() { return; }
    ...
    pub fn update(&mut self, dt: f32) {
        if self.capture != CaptureMode::Fly { return; }
        let forward = self.forward();
        let right   = self.right();
        let speed   = self.fly_speed * if self.shift { 4.0 } else { 1.0 };

        let mut vel = Vec3::ZERO;
        if self.key_fwd   { vel += forward; }
        if self.key_back  { vel -= forward; }
        if self.key_right { vel += right; }
        if self.key_left  { vel -= right; }
        if self.key_up    { vel += Vec3::Y; }
        if self.key_down  { vel -= Vec3::Y; }
