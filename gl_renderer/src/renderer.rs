...
use crate::arena::{
    BoundingSphere, DrawCommand, GpuArena, GpuMaterial, InstanceData, MaterialRegistry, MeshAlloc,
    MeshKey, PackedVertex,
};
...
impl Renderer {
    pub unsafe fn new(gl: &Context, mdi_count: MultiDrawElementsIndirectCountFn) -> Self {
        println!(
            "GL: {} | {}",
        ...
        let main_shader =
            Shader::new_graphics(gl, &src.join("shader.vert"), &src.join("shader.frag"));
        ...
        Self {
            ...
            culled_instance_buf,
            culled_indirect_buf,
            culled_instance_buf_p2,
            culled_indirect_buf_p2,
            bounding_buf,
            batch_index_buf,
            batch_base_buf,
            visible_phase1_buf,
    ...
    unsafe fn upload_material_to_gpu(&mut self, gl: &Context, idx: u32, mat: GpuMaterial) {
        if self.registry.len() > self.material_buf.capacity {
            self.material_buf
                .grow(gl, self.material_buf.capacity * 2, BufferUsage::StaticDraw);
        }
        self.material_buf.upload_subrange(gl, &[mat], idx as usize);
    }
    ...
    unsafe fn allocate_hzb(&mut self, gl: &Context, width: u32, height: u32) {
        ...
        let clear_fbo = gl
            .create_framebuffer()
            .expect("failed to create HZB clear FBO");
    ...
    pub unsafe fn render(
        ...
        gl.begin_query(
            glow::PRIMITIVES_SUBMITTED,
            self.stat_queries[write_set][QI_PRIMS],
        );
        gl.begin_query(
            glow::VERTEX_SHADER_INVOCATIONS,
            self.stat_queries[write_set][QI_VERT],
        );
        gl.begin_query(
            glow::FRAGMENT_SHADER_INVOCATIONS,
            self.stat_queries[write_set][QI_FRAG],
        );
        ...
        for (_entity, (xf, mesh, mat)) in world
            .query::<(&Transform, &Mesh, Option<&Material>)>()
            .iter()
        {
        ...
        for b in 1..m {
            self.scratch_batch_base[b] =
                self.scratch_batch_base[b - 1] + self.scratch_batch_sizes[b - 1];
        }
        ...
        for (b, mesh) in self.scratch_batch_meshes.iter().enumerate() {
            self.scratch_commands
                .push(mesh.draw_command_instanced(0, self.scratch_batch_base[b]));
        }
        ...
        // ── Upload source data ────────────────────────────────────────────────
        self.instance_buf
            .upload_subrange(gl, &self.scratch_instances, 0);
        self.bounding_buf
            .upload_subrange(gl, &self.scratch_bounds, 0);
        self.batch_index_buf
            .upload_subrange(gl, &self.scratch_batch_indices, 0);
        self.batch_base_buf
            .upload_subrange(gl, &self.scratch_batch_base, 0);
        // Both phases start with instance_count = 0.
        self.culled_indirect_buf
            .upload_subrange(gl, &self.scratch_commands, 0);
        self.culled_indirect_buf_p2
            .upload_subrange(gl, &self.scratch_commands, 0);
        ...
        self.visible_phase1_buf
            .upload_subrange(gl, &self.scratch_zeros, 0);
        ...
        self.cull_shader
            .set_vec4_slice(gl, "u_frustum_planes[0]", bytemuck::cast_slice(&planes));
        self.cull_shader.set_mat4(gl, "u_view_proj", &view_proj);
        self.cull_shader
            .set_vec2(gl, "u_screen_size", width as f32, height as f32);
        self.cull_shader
            .set_u32(gl, "u_hzb_max_level", hzb_max_level);
        ...
        let (primitives_submitted, vertex_invocations, fragment_invocations, gpu_time_ms) = if self
            .query_frame
            > 0
        {
            (
                gl.get_query_parameter_u32(
                    self.stat_queries[read_set][QI_PRIMS],
                    glow::QUERY_RESULT,
                ) as u64,
                gl.get_query_parameter_u32(self.stat_queries[read_set][QI_VERT], glow::QUERY_RESULT)
                    as u64,
                gl.get_query_parameter_u32(self.stat_queries[read_set][QI_FRAG], glow::QUERY_RESULT)
                    as u64,
                gl.get_query_parameter_u32(self.stat_queries[read_set][QI_TIME], glow::QUERY_RESULT)
                    as f64
                    / 1_000_000.0,
            )
        } else {
